...
class SocketHandler {
    ...
    this.io.on('connection', (socket) => {
      ...
      socket.on('authenticate', async (data) => {
        try {
          ...
          // routes/auth.js signs { userId }; some other paths sign { id }. Accept both.
          const userId = decoded.id || decoded.userId;

          // Look the user up so we always have an accurate digitalId/role,
          // instead of trusting the client-supplied userType alone.
          const dbUser = await User.findById(userId).select('digitalId role name');
          if (!dbUser) {
            return socket.emit('auth_error', { message: 'Authentication failed - user not found' });
          }

          socket.userId = String(dbUser._id);
          socket.userType = userType || (dbUser.role === 'tourist' ? 'tourist' : 'admin');
          socket.digitalId = dbUser.digitalId;

          // Join rooms used by the new Trip/Location/Geofence/Incident features
          socket.join(`user:${socket.userId}`);
          if (socket.userType !== 'tourist') {
  ...
  // ---- New real-time events used by Trip/Location/Geofence/Incident features ----

  /** Feature 4/5: broadcast a live location update (with trip + zone status) to authorities */
  broadcastLiveLocation(payload) {
    this.broadcastToAdmins('live_location', payload);
  }

  /** Feature 6: broadcast a SAFE/WARNING/VIOLATION status change to authorities and the tourist */
  broadcastGeofenceStatus(touristId, payload) {
    this.broadcastToAdmins('geofence_status', payload);
    this.sendToUser(touristId, 'geofence_status', payload);
  }

  /** Feature 9/10/14: broadcast any new/updated incident (SOS, geofence, AI risk, battery) */
  broadcastIncident(incident) {
    this.broadcastToAdmins('incident_event', incident);
  }

  /** Feature 3: broadcast trip lifecycle changes (started/stopped) to authorities */
  broadcastTripEvent(payload) {
    this.broadcastToAdmins('trip_event', payload);
  }

  /** Feature 8: broadcast offline->online sync completion for a tourist */
  broadcastSyncStatus(touristId, payload) {
    this.broadcastToAdmins('sync_status', payload);
    this.sendToUser(touristId, 'sync_status', payload);
  }
