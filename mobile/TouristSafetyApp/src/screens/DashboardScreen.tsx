...
const DashboardScreen: React.FC<NavigationProps> = ({ navigation }) => {
  ...
  // If the app was closed/reopened mid-trip, Feature 3 requires we don't lose
  // the ACTIVE trip - resume tracking against it instead of losing state.
  const restoreActiveTrip = async () => {
    try {
      const { trip } = await tripAPI.getActive();
      if (trip && trip.status === 'ACTIVE') {
        setIsTrackingLocation(true);
      }
    } catch (error) {
      console.log('No active trip to restore (or offline):', error);
    }
  };
  ...
  const toggleLocationTracking = async () => {
    if (isTrackingLocation) {
      try {
        ...
        Alert.alert('Trip Stopped', 'Location tracking stopped and trip history saved.');
      } catch (error: any) {
        console.error('Error stopping trip:', error);
        Alert.alert('Error', error?.message || 'Failed to stop the trip');
      }
      return;
    }

    try {
      // Mirror raw GPS fixes onto this screen's "Current Location" card while
      // tripTrackingService owns the actual GPS watcher + backend sync.
      tripTrackingService.setUiListener((newLocation) => {
        setLocation(newLocation);
        socketService.sendLocationUpdate(newLocation);
      });
      const tripId = await tripTrackingService.startTrip();
      setIsTrackingLocation(true);
      Alert.alert(
        'Trip Started',
        `Trip ${tripId} started. Your location will be shared with authorities for safety monitoring, including geo-fence and offline-sync protection.`,
        [{ text: 'OK' }]
      );
    } catch (error: any) {
      console.error('Error starting trip:', error);
      Alert.alert('Trip Error', 'Failed to start trip: ' + (error?.message || 'Unknown error'));
    }
  };
        ...
        <View style={styles.actionRow}>
          ...
          >
            ...
            <Text style={styles.actionButtonText}>
              {isTrackingLocation ? 'Stop Trip' : 'Start Trip'}
            </Text>
          </TouchableOpacity>
        </View>
      </View>
