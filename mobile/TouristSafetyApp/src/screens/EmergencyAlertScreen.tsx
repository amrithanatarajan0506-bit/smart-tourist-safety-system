...
import { emergencyAPI, incidentAPI, tokenManager } from '../services/api';
...
const EmergencyAlertScreen: React.FC<NavigationProps> = ({ navigation }) => {
  ...
  const confirmSendAlert = async () => {
    ...
    try {
      ...
      // Feature 9: also record this as a unified Incident (SOS) - this is what
      // powers the Authority Dashboard's Incident Management panel, the
      // acknowledge/respond/resolve workflow, and the tamper-evident ledger entry.
      try {
        await incidentAPI.sos({
        ...
        console.warn('Unified incident SOS call failed, but socket/legacy alert sent:', incidentError);
      }
