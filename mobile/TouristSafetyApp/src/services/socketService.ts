...
class SocketService {
  ...
  private async authenticate() {
    ...
    try {
      // Re-read the latest stored token in case it changed since the socket was created.
      this.token = await AsyncStorage.getItem('authToken');
      if (!this.token) {
        console.log('No auth token available yet, skipping socket authentication');
        return;
      }
      console.log('Authenticating socket with the logged-in user\'s JWT...');
